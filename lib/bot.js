...
Bot.prototype.implode = function(cb) {
  ...
  $this.rollcall(function(err, emails) {
    if(!err) {
      // remove bot from list of people in room
      var email = _.difference(emails, [ $this._person.emails[0] ]);
      // remove all from room
      async.each(emails, function(email, callback) {
        $this.remove(email, function(err) {
          setTimeout(function() {
            callback(err);
          }, _delay);
        });
...
Bot.prototype.subscribe = function(name, fn, cb) {
  ...
  if(!found) {
    $this.subscriber.attach(name, fn, function(err, id) {
      if(!err) {
        debug('attached subscriber to room: %s', $this._room.title);
        // track subsciption id on bot object
        $this._subscriptions.push({name: name, id: id});
      } else {
        debug('could not attach subscriber to room: %s (subscriber does not exist)', $this._room.title);
      }
      cb ? cb(err) : null;
    });
  } else {
    debug('could not attach subscriber to room: %s (subscription already exists)', $this._room.title);
...
Bot.prototype.unsubscribe = function(name) {
  ...
  var subscription = _.find($this._subscriptions, ['name', name]);

  // if subscribed
  if(subscription) {
    debug('detatched subscriber from room: %s', $this._room.title);
    // detach bot from subscriber
    $this.subscriber.detach(subscription.id);
  } else {
    debug('could not detach subscriber from room: %s (subscription does not exist)', $this._room.title);
  }

  // remove subscription reference from room
  $this._subscriptions = _.reject($this._subscriptions, ['name', name]);
