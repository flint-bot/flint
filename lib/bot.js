   ...
   // create room
    .then(room => {
      ...
      return poll(bot, 100, isReady)
        .then(bot => {
          if(!bot) {
            return when.reject(new Error('bot spawn timed out when creating a new room'));
          } else {
            newRoomBot = bot;
            newRoom = room;
            return when(bot);
...
Bot.prototype.newTeamRoom = function(name, emails) {
  ...
  if(this.isTeam) {
    ...
    return when.reject(new Error('this bot is not part of a spark team'));
  }
...
Bot.prototype.unmoderateRoom = function() {
  if(!this.isGroup || this.isTeam) {
    ...
    return when.reject(new Error('bot is not a moderator'));
  }
    ...
    return this.getModerators()
      .then(moderators => {
        ...
        var batch = _.map(moderators, m => {
          return () => this.moderatorClear(m.personEmail);
        });
...
Bot.prototype.moderatorSet = function(email) {
  ...
  if(!this.isGroup || this.isTeam) {
    ...
    return when.reject(new Error('bot is not moderator'));
  }
 ...
 * 
 * @function
 * @param {String} url - URL of file to send to room
 * @returns {Promise.<Message>}
 * 
 * @example
 * flint.hears('/file', function(trigger, bot) {
 ...
 * });
 */
Bot.prototype.file = function(url) {
  if(validator.isUrl(url)) {
    return this.spark.messageSendRoom(this.room.id, { file: url });
  } else {
    return when.reject(new Error('invalid formatted url'));
  }
};
 ...
 * 
 * @function
 * @param {Stream} stream - Stream Readable
 * @returns {Promise.<Message>}
 * 
 * @example
 * flint.hears('/file', function(trigger, bot) {
 ...
 * });
 */
Bot.prototype.upload = function(stream) {
  if(stream instanceof Stream) {
    return this.spark.messageStreamRoom(this.room.id, { stream: stream });  
  } else {
