...
function Bot(flint) {
  ...
  self.log = [];
  self.files = [];
  self.maxLogSize = flint.config.maxLogSize;
  ...
  self.once('start', function() {
    // grab existing room message and add to log
    self.getMessages(function(err, messages) {
      while(!err && messages.length > 0) {
        self.log.push(self.parseMessage(messages.shift()));
      }
      self.emit('log-initialized');
    });
  });
...
Bot.prototype.parseMessage = function(message) {
  ...
  if(message.text && typeof message.text === 'string') {
    // record file urls
    _.forEach(message.files, function(file) {
...
// log message object
Bot.prototype.logMessage = function(message) {
  var self = this;
  
  // truncate message log if maxLogSize != 0
  if(self.maxLogSize && self.maxLogSize !== 0 && self.log.length > self.maxLogSize) {
    self.log = self.log.slice(0, self.maxLogSize);
  }
  
  // truncate file log if maxLogSize != 0
  if(self.maxLogSize && self.maxLogSize !== 0 && self.files.length > self.maxLogSize) {
    self.files = self.files.slice(0, self.maxLogSize);
  }

  // parse message object
  message = self.parseMessage(message);

  // log message to front of array
  self.log.unshift(message);
...
Bot.prototype.getMessages = function(count, cb) {
  ...
  cb = args.pop();
  // optional args
  count = (args.length > 0 && typeof args[0] === 'number') ? args.shift() : self.maxLogSize;
...
Bot.prototype.subscribe = function(name, fn, cb) {
  ...
  if(!found) {
    self.subscriber.attach(name, fn, function(err, id) {
      if(!err) {
        debug('attached subscriber to room: %s', self.myroom.title);
        // track subsciption id on bot object
        self._subscriptions.push({name: name, id: id});
      } else {
        debug('could not attach subscriber to room: %s (subscriber does not exist)', self.myroom.title);
      }
      cb ? cb(err) : null;
    });
  } else {
    debug('could not attach subscriber to room: %s (subscription already exists)', self.myroom.title);
...
Bot.prototype.unsubscribe = function(name) {
  ...
  // if subscribed
  if(subscription) {
    debug('detatched subscriber from room: %s', self.myroom.title);
    // detach bot from subscriber
    self.subscriber.detach(subscription.id);
  } else {
    debug('could not detach subscriber from room: %s (subscription does not exist)', self.myroom.title);
  }
