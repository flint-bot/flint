...
class Bot extends EventEmitter {
    ...
    const bot = {
      room: {
        ...
        exit: () => this.flint.spark.membershipRemove(membership.id)
          .then(() => this.flint.bot.destroy(room)),
      ...
      message: {
        say: (text, file) => {
          ...
          const asText = (tx, fi) => {
            ...
            return this.flint.spark.messageSend({ roomId: room.id, text: tx });
          };
          ...
          const asMarkdown = (md, fi) => {
            ...
            return this.flint.spark.messageSend({ roomId: room.id, markdown: md });
          };

          // if bot.message.say('message') format received...
          if (typeof text === 'string') { // TODO verify file
        ...
        send: (email) => {
          ...
          const asText = (tx, fi) => {
            if (typeof tx === 'string') {
              ...
              return this.flint.spark.messageSend({ toPersonEmail: email, text: tx });
            }
          ...
          const asMarkdown = (md, fi) => {
            if (typeof md === 'string') {
              ...
              return this.flint.spark.messageSend({ toPersonEmail: email, markdown: md });
            }
            return when.reject(new Error('invalid or missing text argument'));
          };
  ...
  destroy(room) {
    // clean up operations
    return this.flint.storage.delete(room.id);
  }
