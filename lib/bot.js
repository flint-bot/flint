...
Bot.prototype.censor = function(messageId) {
  return this.flint.getMessage(messageId)
    .then(message => {
      ...
      if((this.isLocked && this.isModerator) || message.personEmail === this.flint.email) {
        return this.spark.messageRemove(messageId);
      } else {
        return when.reject(new Error('can remove a message if bot is not author or moderator'));
      }
    });
};
