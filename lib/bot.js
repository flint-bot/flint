...
Bot.prototype.say = function(format, message) {
  ...
  var formatDefined = (args.length > 1 && typeof args[0] === 'string' && typeof args[1] === 'string');
  
  // if format defined in function arguments, overide default
  // first argument should be one of the valid formats
  if(formatDefined) {
    if(_.includes(['text', 'markdown', 'html'], _.toLower(args[0]))) {
...
Bot.prototype.dm = function(email, format, message) {
  ...
  // if message is string
    ...
    // send constructed message object to person
    return this.spark.messageSendPerson(email, messageObj);
  }
