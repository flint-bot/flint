...
class Bot {
  ...
  build(_room, _membership, _testTrackingId = '') {
    const room = _room;
    const membership = _membership;
    // if the the trigger that asked for this bot instance has a testTrackingId 
    // we will set in in the header of any POST, PUT or DELETE requests this bot sends to spark
    const testTrackingId = _testTrackingId;
    ...
    const bot = {
      ...
      room: {
         ...
         * bot.exit();
         */
        exit: () => this.flint.spark.membershipRemove(membership.id, testTrackingId)
        ...
        add: (emails) => {
          ...
          const addEmail = (email) => {
            ...
            if (this.flint.validator.isEmail(personEmail)) {
              ...
              return this.flint.spark.membershipAdd({ personEmail, roomId }, testTrackingId)
                .delay(200)
          ...
          const addEmails = (emailArray) => {
            ...
            return when.map(personEmails, personEmail => addEmail(personEmail).catch((err) => {
              ...
              if (successfulAdds.length < results.length) {
                const addEmailsError = new Error('all emails were not sucessfully added ' +
                  '(see err.status)');
              ...
              return this.flint.spark.membershipsGet({ personEmail, roomId })
                .then((memberships) => {
                  if (memberships instanceof Array && memberships.length === 1) {
                    return this.flint.spark.membershipRemove(memberships[0].id, testTrackingId);
                  }
                  return when.reject(new Error(`invalid email " +
                    ${email}" was used in bot.remove()`));
                })
                .delay(200)
                .then(() => when(personEmail));
            }
          ...
          const removeEmails = (emailArray) => {
            ...
            return when.map(personEmails, personEmail => removeEmail(personEmail).catch((err) => {
              ...
              if (successfulRemoves.length < results.length) {
                const removeEmailsError =
                  new Error('all emails were not sucessfully removed (see err.status)');
      ...
      say: (text, file) => { // TODO process file
        ...
        const asText = () => this.flint.spark.messageSend(
          { roomId: room.id, text: text }, file, testTrackingId);

        // send message as markdown with optional file
        const asMarkdown = () => this.flint.spark.messageSend(
          { roomId: room.id, markdown: text }, file, testTrackingId);
