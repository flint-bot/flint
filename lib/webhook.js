/**
 * @file Defines Webhook Processor for Spark
 ...
 *
 * @param {Object} flint
 * The flint object this function applies to.
 *
 * @returns {Function}
...
function Webhook(flint) {
  ...
  return function(req, res, next) {
    ...
    // get webhook content
    var id = req[flint.options.webhookRequestJSONLocation].id;
    var name = req[flint.options.webhookRequestJSONLocation].name;
    var resource = req[flint.options.webhookRequestJSONLocation].resource;
    var event = req[flint.options.webhookRequestJSONLocation].event;
    var data = req[flint.options.webhookRequestJSONLocation].data;
    var roomId = req[flint.options.webhookRequestJSONLocation].filter ? req[flint.options.webhookRequestJSONLocation].filter.split('=')[1] : null;
