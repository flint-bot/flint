...
class Flint extends EventEmitter {
  ...
  constructor(config) {
    ...
    this.config.token = config.token || process.env.FLINT_API_TOKEN || process.env.TOKEN || null;
    this.config.webhookSecret = config.webhookSecret || process.env.FLINT_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET || config.webhookSecret || 's3cret';
    this.config.webhookUrl = config.webhookUrl || process.env.FLINT_WEBHOOK_URL || process.env.WEBHOOK_URL || null;
  ...
  use(pluginType, PluginConstructor) {
    ...
    if (_.includes(validPluginTypes, _.toLower(pluginType)) && typeof PluginConstructor === 'function') {
      const newPlugin = new PluginConstructor(this);
      // validate authorization
      if (_.toLower(pluginType) === 'authorization') { // TODO
        this.authorization = newPlugin;
      }

      // validate logger
      if (_.toLower(pluginType) === 'logger') { // TODO
        this.logger = newPlugin;
      }

      // validate storage
      if (_.toLower(pluginType) === 'storage'
        && typeof newPlugin.create === 'function'
        && typeof newPlugin.read === 'function'
        && typeof newPlugin.delete === 'function'
      ) {
        this.storage = newPlugin;
      }

      // validate listener
      if (_.toLower(pluginType) === 'listener') { // TODO
        this.listener = newPlugin;
      }
    } else {
  ...
  message(email, text, file) { // TODO process file
    if (!email) {
