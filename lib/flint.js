  ...
  function processWebhook(req) {
    ...
    self.sparky.message.get(req.params.data.id, function(err, message) {
      ...
      if(trigger.message.text) {
        // trim whitespace
        trigger.message.text = trigger.message.text.trim();
      } else {
        // no text param received
        trigger.message.text = '';
      }
      
      // emit events
      self.emit('message', trigger.message, bot, trigger);
      if(trigger.files) self.emit('files', trigger.files, bot, trigger);
