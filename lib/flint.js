...
class Flint extends EventEmitter {
          ...
          .then((memberships) => {
            ...
            if (roomMembership) {
              return this.spark.roomGet(query.roomId)
                .then(room => this.bot.build(room, roomMembership, testTrackingId));
            }
  ...
  message(email, text, file) { // TODO process file
    ...
    const asText = () => this.spark.messageSend({ toPersonEmail: email, text: text })
      .then(message => this.query(message));

    // send message as markdown
    const asMarkdown = () => this.spark.messageSend({ toPersonEmail: email, markdown: text })
      .then(message => this.query(message));
