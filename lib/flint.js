  ...
  if(!this.active) {
    ...
    return this.spark.personMe()
      ...
      .then(person => {
        ...
        if(this.person.domain === 'sparkbot.io' || this.person.domain === 'webex.bot') {
      ...
      .then(() => {
        if (this.options.webhookUrl) {
          ...
          this.getWebhooks()
            ...
            .then(() => {
              if(this.options.webhookUrl) {
                this.spark.webhookAdd('all', 'all', u.base64encode(this.options.webhookUrl.split('/')[2] + ' ' + this.email))
                  .then(webhook => {
                    this.webhook = webhook;
                    return this.spark.webhookAdd('attachmentActions', 'created', u.base64encode(this.options.webhookUrl.split('/')[2] + ' ' + this.email))
                      .then(webhook => {
          ...
          this.websocket = new Websocket(this, webhook);
 ...
 * @function
 * @memberof Flint
 * @param {String} attachmentActionId - attachmentActionID from Spark API.
 * @returns {Promise.<AttachmentAction>}
 */
Flint.prototype.getAttachmentAction = function(attachmentActionId) {
  return this.spark.attachmentActionGet(attachmentActionId);
};
 ...
 * @memberof Flint
 * @private
 * @param {Object} attachmentAction - Spark attachentAction  Object
 * @returns {Promise}
 */
Flint.prototype.onAttachmentActions = function(attachmentAction) {
  var bot = _.find(this.bots, bot => bot.room.id === attachmentAction.roomId);
  if(bot) bot.lastActivity = moment().utc().toDate();

  // if bot found...
  if(bot) {
    this.emit('attachmentAction', bot, attachmentAction, this.id);
  // else, bot not found...
  } else {
    return when(false);
  }
...
Flint.prototype.spawn = function(roomId, actorId) {
  ...
  return this.getRoom(roomId)
    ...
    .then(memberships => {
      ...
      .then(
          () => {
              // stores the email address of who added the bot
              if (undefined !== actorId && actorId ) {
                  this.spark.personGet(actorId).then((p) => {
...
function cleanupListeners(flint) {
  // Cleanup webhooks or websockets
  if (flint.options.webhookUrl) {
    return flint.getWebhooks()
      // get webhooks
      .then(webhooks => {
    ...
    return flint.websocket.cleanup()
      .then(() => {
        delete flint.websocket;
        return when(true);
      });
  }
}