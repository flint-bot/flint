...
Flint.prototype.botSpawner = function(roomId, callback) {
  ...
  var roomId = roomId; // room-object that bot was added to
  var webhook;
  ...
  async.series([
    ...
    function(cb) {
      ...
      $this.sparky.webhooks.get(function(err, webhooks) {
        var _webhook = _.find(webhooks, function(webhook) {
...
Flint.prototype.hears = function(phrase, options, action) {
  ...
  var action = args.pop();
  // optional args
  var options = (args.length > 0 && typeof args[0] === 'object') ? args.shift() : null;
