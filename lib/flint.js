  ...
  if(!this.active) {
    ...
    return this.spark.personMe()
      ...
      .then(person => {
        ...
        if(/.*\(bot\)$/.test(this.person.displayName)) {
          this.machine = true;
        }
...
Flint.prototype.getTrigger = function(messageId) {
   ...
   * @property {array} args - Filtered array of words in message text. If regex match, array will include all words. If phrase match, array will include all words.
  ...
  if(bot) {
    return this.getTrigger(message.id)
      .then(trigger => {
        ...
        function runActions(matched, trigger, id) {
          _.forEach(matched, lex => {
            ...
            // for string
              ...
              trigger.args = trigger.text.split(' ');
              trigger.args = trigger.args.slice(trigger.args.indexOf(lex.phrase));
              trigger.phrase = _.toLower(lex.phrase);
              lex.action(bot, trigger, id);
              return true;
            } 
        ...
        if(trigger.text) {
          ...
          var matched =  _.filter(this.lexicon, lex => {
            ...
            // if lex.phrase is string and this is NOT a machine account
            else if(!this.machine && lex.phrase && typeof lex.phrase === 'string' && lex.phrase === _.toLower(trigger.text).split(' ')[0]) {
              return true;
            }
            
            // if lex.phrase is string and this is a machine account
            else if(this.machine && lex.phrase && typeof lex.phrase === 'string') {
              var regexPhrase = new RegExp('(^| )' + lex.phrase.replace(/([\.\^\$\*\+\?\(\)\[\{\\\|])/g, '\\$1') + '($| )');
        ...
        if(matched && typeof this.authorize === 'function') {
          ...
          return when(this.authorize(bot, trigger, this.id))
            .then(authorized => {
              ...
              //if authorized
              if(authorized) {
                runActions(matched, trigger, this.id);
                return when(trigger);
              } else {
...
Flint.prototype.hears = function(phrase, action, helpText) {
  ...
  else if(phrase instanceof RegExp && typeof action === 'function' && (typeof helpText === 'undefined' || typeof helpText === 'string')) {
    this.lexicon.push({ 'id': id, 'phrase': phrase, 'action': action, 'helpText': helpText});
    return id;
  }
