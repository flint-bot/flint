...
function Flint(config) {
  ...
  // set webhook url
  $this._webhookUrl = $this.config.hookUrl + ':' + $this.config.remotePort;

  // set webhook path
  $this._webhookResource = '/spark-' + utility.genRand(8);

  // node sparky init
  function initSparky(token) {
    $this.sparky = nodesparky({
      token: token,
      webhook: $this._webhookUrl + $this._webhookResource
    });
  }
  ...
  // function to start Flint services
  function startFlint() {
    // start route subscriber
    ...
    $this.monitor($this);
    // listen for webhooks
    $this.server.route('post', $this._webhookResource, processWebhook);
  }
...
Flint.prototype.publish = function(name, cb) {
  ...
  $this.subscriber.create(name, cb);
};

// exposes an internal url
Flint.prototype.expose = function(internalUrl, filename, cb) {
  var $this = this;

  $this.proxy.create(internalUrl, filename, cb);
};
