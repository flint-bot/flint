      ...
      .then(trigger => {
        ...
        if(trigger.text) {
          ...
          var matched =  _.filter(this.lexicon, lex => {
            ...
            var isRegexMatch = (lex.phrase && lex.phrase instanceof RegExp && lex.phrase.test(trigger.text));
            
            var isStringMatch = false;

            // if machine account convert lex.phrase string to regex
            if(this.machine && typeof lex.phrase === 'string') {
              var re = new RegExp('(^| )' + lex.phrase.replace(/([\.\^\$\*\+\?\(\)\[\{\\\|])/g, "\\$1") + '($| )','i');
              isStringMatch = re.test(trigger.text);
            } 
            
            // else match lex.phrase against first word in mssage.text
            else {
              isStringMatch = (lex.phrase && typeof lex.phrase === 'string' && lex.phrase === _.toLower(trigger.text).split(' ')[0]);
            }
