 ...
 * @property {boolean} machine - Is Flint attached to Spark using a machine account?
...
function Flint(options) {
  ...
  this.options.removeWebhooksOnStart = typeof this.options.removeWebhooksOnStart === 'undefined' ?  true : this.options.removeWebhooksOnStart;
  ...
  if(!this.active) {
    ...
    return this.spark.personMe()
      ...
      .then(person => {
        ...
        // check if account is machine account
        if(person.domain === 'sparkbot.io') {
          this.machine = true;
        }
  ...
  return this.spark.membershipsGet()
    .then(memberships => {
      ...
      var batch = _.map(memberships, m => {
        return () => this.spawn(m.roomId).catch(err => this.debug(err.message));
      });
...
Flint.prototype.auditBots = function() {
  ...
  // if not using webhook, manually add bots to new rooms and return
  if(this.auditCounter === 0 && !this.webhook) {
    return this.getMemberships()
      .then(memberships => {
        var botRoomMemberships = _.map(this.bots, bot => bot.membership);
        var createdRoomMemberships = _.differenceBy(memberships, botRoomMemberships, 'id');

        return when.map(createdRoomMemberships, membership => this.onMembershipCreated(membership));
      });
  }
  ...
  if(this.auditCounter % 5 === 0) {
    ...
    _.forEach(botsToRemove, bot => this.despawn(bot.room.id));
  }
  
  // check for zombies
  if(this.auditCounter % 27 === 0) {
    this.getRooms()
      .then(rooms => {
  ...
  return when.map(this.bots, bot => {
    ...
    if(this.initialized && bot.auditTrigger === this.auditCounter) {
      ...
      var memberships = () => this.getMemberships(bot.room.id)
        .then(memberships => {
          return when.map(memberships, membership => this.onMembershipUpdated(membership))
            .then(() => {

              // if not using webhook, manually create/delete memberships
              if(!this.webhook) {
                var roomMemberships = _.differenceBy(memberships, [ bot.membership ], 'id');
                var createdMemberships = _.differenceBy(roomMemberships, bot.memberships, 'id');
                var deletedMemberships = _.differenceBy(bot.memberships, roomMemberships, 'id');

                var createMemberships = when.map(createdMemberships, membership => this.onMembershipCreated(membership));
                var deleteMemberships = when.map(deletedMemberships, membership => this.onMembershipDeleted(membership));

                return when.all([createMemberships, deleteMemberships])
                  .then(() => when(memberships));
              }
...
Flint.prototype.getTrigger = function(messageId) {
   ...
   * @property {string} id - Message ID
   * @property {string} text - Message Text
   * @property {string} html - Message HTML
   * @property {string} markdown - Message Markdown
   * @property {(string|regex)} phrase - Matched lexicon phrase
   * @property {array} files - Message Files
   * @property {array} args - Message Text as array
  ...
  return this.getMessage(messageId)
    .then(message => {
      ...
      trigger.text = message.text || null;
      trigger.html = message.html || null;
      trigger.markdown = message.markdown || null;
      trigger.created = message.created;
      trigger.args = message.text ? message.text.split(' ') : [];
      ...
      var files = this.parseFile(message)
        .then(message => {
           trigger.files = message.files || null;
           return when(true);
        });
...
Flint.prototype.getPerson = function(personEmail) {
  return this.spark.personByEmail(personEmail)
    .then(person => {
      ...
      person.userame = _.split(person.email, '@', 2)[0];
    ...
    return this.getTrigger(message.id)
      .then(trigger => {
        ...
        if(trigger.text) {
          ...
          var matched =  _.find(this.lexicon, lex => {
            
            // check if lex.phase is regex and matches trigger.text
            var isRegexMatch = (lex.phrase && lex.phrase instanceof RegExp && lex.phrase.test(_.toLower(trigger.text)));
            
            // check if lex.phase is string and matches aginst first word of trigger.text
            var isStringMatch = (lex.phrase && typeof lex.phrase === 'string' && lex.phrase === _.toLower(trigger.text).split(' ')[0]);
            ...
            return when(this.authorize(bot, trigger, this.id))
              .then(authorized => {
                ...
                if(authorized) {
                  ...
                  // run command
                  matched.action(bot, trigger, this.id);
      ...
      .then(trigger => {
        ...
        if(trigger) {
           ...
           * @property {string} id - Flint UUID
           */
          this.emit('hears', bot, trigger, this.id);
...
Flint.prototype.spawn = function(roomId) {
  ...
  return this.getRoom(roomId)
    ...
    .then(() => this.getMemberships(newBot.room.id))
    .then(memberships => {
      ...
      if(botMembership) {
        ...
        return when.reject(new Error('bot could not spawn, membership not found in room: ' + roomId));
      }
    })
 ...
 * @param {String} [helpText] - The string of text that describes how this command operates.
 * @returns {String}
 * 
 * @example
 ...
 * });
 */
Flint.prototype.hears = function(phrase, action, helpText) {
  if((typeof phrase === 'string' || phrase instanceof RegExp) && typeof action === 'function' && (typeof helpText === 'undefined' || typeof helpText === 'string')) {
    var id = u.genUUID64();
    this.lexicon.push({ 'id': id, 'phrase': phrase, 'action': action, 'helpText': helpText});
    return id;
  } else {
    return false;
  }
...
Flint.prototype.showHelp = function(header, footer) {
  ...
  _.forEach(this.lexicon, lex => {
    if(lex.helpText) {
      helpText = helpText + lex.phrase + ' - ' + lex.helpText + '\n';
    }
  });

  helpText = header + helpText + footer;
...
Flint.prototype.use = function(pluginPath) {
  if(path.parse(pluginPath).ext === '.js') {
    try {
      ...
      this.debug('Loading plugin at "%s"', pluginPath);
      return true;
    }

    catch(err) {
      this.debug('Could not load plugin at "%s"', pluginPath);
      return false;
    }
  }
};
