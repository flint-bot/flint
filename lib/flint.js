...
// default plugins
const MemStore = require('../plugins/storage/memory');
...
class Flint extends EventEmitter {
  ...
  constructor(config) {
    ...
    if (!this.config.token || !this.config.webhookUrl) {
  ...
  onStarted() {
    ...
    if (this.storage
      && _.has(this.storage, 'start')
      && typeof this.storage.start === 'function'
    ) {
      this.storage.start()
  ...
  onStopped() {
    ...
    if (this.storage
      && _.has(this.storage, 'stop')
      && typeof this.storage.stop === 'function'
    ) {
      this.storage.stop()
              ...
              if (foundWebhooks && foundWebhooks instanceof Array && foundWebhooks.length > 0) {
                return when.map(webhooks, webhook => this.spark.webhookRemove(webhook.id))
                  .catch((err) => {
                    ...
                    this.logger.error(new Error(`could not remove all webhooks with name: ${webhookName}`));
                    return when(true);
                  });
              }
              return when(true);
  ...
  // start flint and initalize listeners
  start() {
    if (!this.active) {
  ...
  use(pluginType, PluginConstructor) {
    const validPluginTypes = ['authorization', 'logger', 'storage'];
  ...
  // set spark token and reinit Spark
  setSparkToken(token) {
    if (token && typeof token === 'string') {
      this.config.token = token;
      this.spark = new Spark(this.config);
      return when(this.config.token);
    }
      ...
      if (_.has(query, 'roomId')) {
        return this.spark.membershipsGet()
          .then((memberships) => {
            ...
            if (roomMembership) {
              return this.bot.build(query.roomId, roomMembership.id);
            }
